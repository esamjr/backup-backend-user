...
def forget_pass(request, token):
    if settings.FLAG == 1:
        link = 'https://x-user.mindzzle.com/NewPassword?token=' + token
    elif settings.FLAG == 2:
        link = 'http://staging-user.mindzzle.com/NewPassword?token=' + token
    else:
        link = 'http://localhost:8000/NewPassword?token=' + token
    return link
...
def send_forget_email(request):
    reset_url = settings.SITE_URL + 'NewPassword?token=' + request['token']
...
def send_registration_email(request):
    regis_url = settings.SITE_URL + 'RegisterConfirmation?token=' + request['token']
